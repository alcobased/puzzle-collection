...
const ModalWords = () => {
  ...
  const { lists, activeList, singleWordList } = useSelector(
    (state) => state.words
  );
  ...
  const handleRemoveList = (listId) => {
    if (
      window.confirm(`Are you sure you want to delete word list "${listId}"?`)
    ) {
  ...
  const handleAddWord = (e) => {
    e.preventDefault();
    if (newWord.trim()) {
      if (singleWordList === "multi") {
        dispatch(addWord(newWord.trim()));
      } else {
        dispatch(addSingleWordList(newWord.trim()));
      }
    }
    setNewWord("");
  };
  ...
  const handleAddFromTextArea = () => {
    if (textAreaValue.trim()) {
      ...
      dispatch(addWords(words));
        ...
        dispatch(addSingleWordLists(words));
      }
      setTextAreaValue("");
    }
  };
    ...
    <>
        ...
        <div className="list-controls">
          ...
          {listIds.length > 1 && (
            <button
              ...
              onClick={() => handleRemoveList(activeList)}
            >
              Remove Current List
            </button>
          )}
      ...
      <div className="modal-section">
        ...
        <textarea
          ...
          rows="4"
        ></textarea>
        <button className="modal-button" onClick={handleAddFromTextArea}>
          Add Words
        </button>
      </div>
    </>
